...
def extract_text(file):
    ...
    # Store diagnostics in session_state for access in Analysis page
    ...
    if file.type == "application/pdf":
            ...
            with pdfplumber.open(file) as pdf:
                for idx, page in enumerate(pdf.pages):
                    try:
                        ...
                        page_text = ""
                        extraction_error = f"Poppler/PDFPlumber error: {str(e)}"
                        st.session_state.extraction_logs.append(f"Page {idx+1}: {extraction_error}")
                    ...
                    if page_text and page_text.strip():
                        ...
                        try:
                            ...
                            extraction_error = f"OCR Error: {str(e)}. (Scanned PDFs require poppler-utils)"
                            st.session_state.extraction_logs.append(f"Page {idx+1} OCR: {extraction_error}")
                            ocr_missing = True

        except Exception as e:
            st.session_state.extraction_logs.append(f"Open PDF Error: {str(e)}")
            return ""
        ...
        try:
            ...
            st.session_state.extraction_logs.append(f"Open DOCX Error: {str(e)}")
        ...
        try:
            ...
            if not text.strip(): ocr_missing = True
        except Exception as e:
            st.session_state.extraction_logs.append(f"Image Error: {str(e)}")
            ocr_missing = True
            
    if ocr_missing:
        st.session_state.extraction_logs.append("⚠️ OCR extraction found no text. Scanned document might be unreadable.")
...
with st.sidebar:
    ...
    with st.expander("System Checks"):
        # Check RapidOCR/OpenCV
        if RAPIDOCR_AVAILABLE:
            st.write("RapidOCR: ✅ Found")
        else:
            st.write("RapidOCR: ❌ Missing")
            
        # Check Poppler (needed for PDF to Image)
        poppler_found = True
        try:
            import subprocess
            subprocess.run(["pdftocairo", "-h"], capture_output=True)
        except:
            poppler_found = False
        ...
        if poppler_found:
            ...
            st.caption("Needed for scanned PDFs.")
...
# -------------------------------
    ...
    if words < 50:
        ...
        # Show extraction logs if text is missing or short
