...
# Force Tesseract path once
tess_path = get_tesseract_path()
if tess_path:
    pytesseract.pytesseract.tesseract_cmd = tess_path
else:
    RAPIDOCR_ERROR = "Tesseract binary not found on system. Ensure 'tesseract-ocr' is in packages.txt."
...
def extract_text(file):
            ...
            for page in pdf.pages:
                ...
                if page_text and page_text.strip():
                    ...
                    try:
                        ...
                        ocr_engine, ocr_err = get_ocr_engine()
                        if ocr_engine:
                            # Use pytesseract for the image-based page
                            page_text = pytesseract.image_to_string(im)
                            if page_text:
                                text += page_text + "\n"
                        else:
                            ocr_missing = f"OCR Engine not available. Details: {ocr_err}"
                    except Exception as e:
                        ocr_missing = f"OCR Error: {str(e)}"
    ...
    if ocr_missing:
        st.warning(f"⚠️ OCR processing encountered an error: {ocr_missing}")
        st.info("💡 If this is a deployed server, ensure 'packages.txt' with libgl1 is included.")
            ...
            with st.spinner("🤖 AI is analyzing your resume... Please wait..."):
                try:
                    ...
                    if not extracted_text or len(extracted_text.strip()) < 10:
                        st.error("❌ Failed to extract text from this file. Please check if it's a valid PDF or image.")
                    else:
...
# ===================================
    ...
    # Safety check for missing session data
    if 'text' not in st.session_state or not st.session_state.text:
        st.warning("No resume data found. Please upload again.")
    ...
    with st.sidebar:
        st.title("Debug Info")
        if st.checkbox("Show Raw Extraction"):
            st.text(text[:500] + "...")
    ...
    words = len(text.split())
    if words < 100:
        st.error("🚨 **Error**: The extracted text is very short. Please ensure you uploaded a clear resume file.")
    else:
