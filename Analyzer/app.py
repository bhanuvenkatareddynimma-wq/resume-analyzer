...
with st.sidebar:
    ...
    with st.expander("Diagnostics"):
        ...
        else: st.write("RapidOCR: ❌ Missing (OpenCV/libgl1?)")
        
        # Check Tesseract Binary
        if PYTESSERACT_AVAILABLE:
            try:
                import subprocess
                subprocess.run([pytesseract.pytesseract.tesseract_cmd, "--version"], capture_output=True)
                st.write("Tesseract: ✅ Found")
            except:
                st.write("Tesseract: ❌ Bin Missing")
                st.caption("Install Tesseract engine on Windows.")
        else: st.write("Tesseract: ❌ Missing lib")
